...
class FindingAidPDFtoEAD():
    ...
    def assemble_ead(self):
        ...
        try:
            self.pdfauthor = self.element_tree.xpath('//page[@number="1"]/text[text()[normalize-space(.)="Compiled by"]]')[0].getnext().text.strip()
        except:
            self.pdfauthor = 'Special Collections Staff'

        # date - last node over "20" width on first page - "reformatted" or "revised" dates okay?
        self.pdfdate = self.element_tree.xpath('//page[@number="1"]/text[@width>"20"]')[-1].text.strip()
    ...
    def getrcoldata(self, lcolname):
        ...
        try:
            ...
            afterrcoltop = str(int(self.element_tree.xpath('//page[@number="3"]/text[@left=' + self.xpos_of_left_column + ' and @top=' + rcoltop + ']/following::text[b]')[0].get('top'))-10)
            ...
            try:
                ...
                try:  # split query - test each against different lines and expand the selection
                    ...
                    afterrcoltop = str(int(self.element_tree.xpath('//page[@number="3"]/text[@left=' + self.xpos_of_left_column + ' and @top=' + nextcoltop + ']/following::text[b]')[0].get('top'))-10)
                    datalines = self.element_tree.xpath('//page[@number="3"]/text[@top>' + rcoltopbuffer + 'and @top<' + afterrcoltop + ' and @left>"200"]')
                    for el in datalines:
                        lcoldata.append(el.text.strip())
                    pdfdata = ' '.join(lcoldata)
